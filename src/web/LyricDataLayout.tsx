...
// ─── Shared context type (consumed by child routes via useLyricData) ──────────

export type LyricDataContextValue = {
  props: LyricVideoCompositionProps | null;
  error: string | null;
  splashText: string;
  showSplash: boolean;
};

export const useLyricData = (): LyricDataContextValue =>
  useOutletContext<LyricDataContextValue>();
...
const normalizeManifestTrackIds = (trackIds: unknown): string[] =>
  Array.isArray(trackIds)
    ? Array.from(new Set(trackIds.filter((id): id is string => typeof id === "string")))
    : [];

const buildPlaylists = (library: SongLibraryItem[], manifest: WebManifest): PlaylistSummary[] => {
...
/**
 * Layout route that loads the lyric library and exposes the result to child
 * routes (LyricPlayerPage, EffectLabRoute) via Outlet context.
 *
 * Routes that do NOT need lyric data (StudioHomePage, PaperStudioPage) should
 * NOT be nested under this layout.
 */
export const LyricDataLayout: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const createPlaceholderSong = (song: ManifestSong, nickname: string): SongLibraryItem => ({
      id: song.id,
      title: song.title,
      artist: song.artist,
      lyricOffsetMs: 0,
    ...
    const prefetchMedia = (url: string | undefined) => {
      if (!url || mediaPrefetchCache.has(url)) return;
    ...
    const loadSongData = async (song: ManifestSong, nickname: string): Promise<SongLibraryItem> => {
  ...
  const context = useMemo(
    (): LyricDataContextValue => ({props, error, splashText, showSplash}),
    [props, error, splashText, showSplash],
  );
